...
def test_resolve_prefers_cwd_then_recipe_dir(
    ...
    assert len(notes) == 4 and all("레시피 파일 기준" in n for n in notes)
    assert data["inputs"]["bank"] == "bank"  # 원본 dict 불변
    assert out["output"]["root"] == "out"  # output 은 건드리지 않는다
    # cwd 에 있으면 그대로(하위 호환)
    monkeypatch.chdir(proj)
    out2, notes2 = R.resolve_recipe_paths(data, tmp_path / "elsewhere")
    assert out2["inputs"]["bank"] == "bank" and notes2 == []
    ...
    assert len(notes4) == 1 and notes4[0].startswith("pipeline.placement.roi.path")
...
def test_load_from_other_cwd_cli_and_studio(
    ...
    assert len(notes3) == 2 and rec3.inputs.bank_key() == (proj / "bank").as_posix()
    ...
    assert (tmp_path / "o" / "manifest.csv").is_file()
    # 스튜디오 세션도 같은 규칙 + path_notes
    s = StudioSession()
    s.load(recipe2)
    assert len(s.path_notes) == 2 and os.path.isdir(s.recipe.inputs.bank_key())

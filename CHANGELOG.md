...
### Added
- **자동 정지 + 사람 수정률(학습 루프 T12)** — 자동 루프는 **망가져도 계속 돕니다**. 그래서 세 가지를 보고 라운드를 **열지 않습니다**: ① 몇 라운드째 승급이 없음(데이터를 더 넣어서는 나아지지 않는다는 뜻 — 조명·해상도·모델을 볼 때입니다) ② **사람 수정률**이 바닥(모델이 그린 마스크를 아무도 다듬지 않는 상태 = 모델이 좋아진 게 아니라 **아무도 안 보고 있는** 것일 수 있습니다) ③ 보관함 클래스 **분포가 급히 바뀜**(로트·공정 변화). 기준은 `loop.yaml` 의 `breaker` 에서 정하고 **기본값은 전부 제한 없음**이라 지금까지와 똑같이 돕니다. 멈춘 것은 실패가 아니라 신호이므로 되돌리는 길은 무시 스위치가 아니라 **`anograft loop breaker-reset --note "무엇을 바꿨는지"`** 하나입니다(원장 `rounds.jsonl` 에 지점이 남고, 그 앞 라운드는 다음 판정에서 빠집니다). 진행 중인 라운드는 막지 않고(사람이 판정해 둔 검토 대기가 썩지 않게), 스케줄러가 부르는 `loop tick` 에게 정지는 **오류가 아니라 상태**입니다(종료 코드 0 · `--force` 로는 넘어가지 않습니다). `loop status` 가 지금 무엇을 감시하고 있는지(`연속 유지 1/3회` · `사람 수정률 40%`)를 함께 보여 줍니다.
- **처음 보는 형상 + 미분류 + 기준선 재설정(학습 루프 T15)** — 루프의 가장 약한 고리인 **새 결함 유형**을 놓치지 않기 위한 셋입니다. ① **처음 보는 형상**(`novelty_score`): 결함의 외형(대비·질감·선명도·면적·길쭉함·채움)을 보관함 조각들과 견주어 **어느 것과도 멀면** 검토 대기의 맨 앞으로 올립니다(거리는 그 보관함의 흔한 편차 단위로 재고, 비교할 조각이 다섯 개도 안 되면 판정하지 않습니다 — 작은 보관함에서는 무엇이든 멀어 보입니다). ② **미분류(`__unsorted__`)**: 처음 보는 형상을 채택하면 **이름 없이** 보관함의 미분류 자리로 들어갑니다 — 틀린 이름은 그 클래스를 조용히 오염시키지만 미분류는 `anograft bank promote <보관함> --to <클래스>` 한 줄로 되돌립니다. 미분류는 합성·학습 출력(`data.yaml`)에서 빠지고 **클래스 목록의 맨 끝**에 고정돼 다른 클래스의 id 를 밀지 않습니다(= 기존 모델과 호환). `bank ls`·`run` 이 "미분류 N개는 합성에서 빠집니다"를 알려 줍니다. ③ **클래스 신설은 라운드를 멈춥니다**: 평가셋에 새 클래스의 정답이 없으면 모델이 **잘 잡을수록 헛검출로 집계되어 점수가 떨어지고** 라운드 비교가 끊깁니다. 그래서 자동으로 넘기지 않고 멈춰서 알려 주고, 평가셋을 손본 뒤 **`anograft loop baseline-reset`** 으로 지점을 남기면(원장에 기록) 그 다음 라운드는 점수를 견주지 않고 현재 모델을 새로 세웁니다.

...
> **상태: v0.12.0** — **학습 루프**(`anograft loop run`/`tick` — 스코어링 → 검토 대기 → 사람 판정 → 보관함 편입 → 합성 → 재학습·평가 → 승급을 한 바퀴로, 멱등. 스케줄러가 부르고 Graft 는 상주하지 않는다. **처음 보는 형상 → 미분류** · **자동 정지** · 평가셋 누수 거부 · 학습기는 `trainers.yaml` 로 등록된 **별도 환경의 프로세스** — 루프를 안 쓰면 아래는 그대로다) · CLI 코어(7단계 파이프라인 · 레시피 · 결함 보관함(bank) · YOLO/MVTec/COCO 출력 · 프리셋 10종 · 보관함 없이 도는 self-cut/perlin · 구조 정합 배치 · GrabCut·annulus ROI · VisA·DTD 어댑터)와 **웹 UI `anograft serve`**(브라우저에서 **① 결함 보관함 · ② 결함 표시 · ③ 미리보기 · ④ 일괄 생성 · ⑤ 검수 · ⑥ 학습 루프(현황·라운드 기록 — 루프를 안 쓰면 비어 있다)** 여섯 화면이 **서로 인계된다** — 레시피·조각·최근 경로가 버튼 하나로 넘어간다. 런타임 의존성은 그대로 넷, 서버는 표준 라이브러리) · 기존 창(PySide6 GUI) **병행 유지** — 결함 사진만 있으면 화면만으로 마스크 그리기 → 보관함 정리 → 미리보기 → 일괄 생성 → 검수·정리된 데이터셋까지. 실데이터 1차 적용(경면 금속 원형 부품)에서 나온 [알려진 문제 10건](KNOWN-ISSUES.md)을 v0.6 에서 보정. **2차 실데이터 적용은 아직 미검증**(데이터가 생기면 아래 3줄로 확인). 공개 데이터(MVTec metal_nut)에선 합성 유/무 YOLO mAP50 **0.31 → 0.38**(grabcut 보관함) / **→ 0.45**(hybrid 보관함) — 같은 분할·학습 시드 3개 평균(`BENCHMARKS.md`; Magnetic Tile 은 한 프리셋으론 break +0.23 · blowhole −0.30 으로 갈려 합계 −0.02 였지만, blowhole 만 `relative-paste` 로 갈라 `dataset merge` 하니 **0.46 → 0.61(+0.15, 3/3)** — 결함 성격별 프리셋). 표준셋 실제 사본(MVTec metal_nut·screw·grid·bottle·hazelnut·carpet · Magnetic Tile · VisA · DTD)으로는 v0.8 에서 리허설·벤치 완료 — `TESTING.md` · `KNOWN-ISSUES.md` 결정 근거 절.
...
> **Status: v0.12.0** — a **training loop** (`anograft loop run`/`tick` — score → review queue → your verdicts → bank intake → synthesis → retrain/evaluate → promote, in one idempotent round called by your scheduler; Graft never stays resident. **Unseen shapes → unsorted**, a **circuit breaker**, eval-set leak refusal, and trainers as **separate processes** registered in `trainers.yaml` — skip the loop and everything below is unchanged) plus the CLI core (7-stage pipeline, recipes, defect bank, YOLO/MVTec/COCO output, 10 presets, bank-free self-cut/perlin sources, structure-aware placement, GrabCut/annulus ROI, VisA/DTD adapters) plus a **web UI (`anograft serve`)** with all six screens in the browser — **① Bank · ② Label · ③ Preview · ④ Batch · ⑤ Review · ⑥ Training loop** (status and round history; empty unless you run the loop), **handing off to each other** — recipes, crops and recent paths move with one button (still four runtime dependencies; the server is pure standard library) — and the **desktop window (PySide6) kept alongside** — with only defect photos you can label → tidy the bank → preview → generate → review and export a pruned set without leaving the UI. The [10 known issues](KNOWN-ISSUES.md) from a first real-data application (mirror-finish round metal part) are addressed in v0.6. **A second real-data pass is still unverified** — three commands once you have data (below). On public data (MVTec metal_nut) synthetic augmentation lifted YOLO mAP50 **0.31 → 0.38** (grabcut bank) / **→ 0.45** (hybrid bank), mean of three training seeds on a fixed split (`BENCHMARKS.md`; on Magnetic Tile one preset diverged per class — break +0.23, blowhole −0.30, net −0.02 — but splitting only blowhole onto `relative-paste` and merging with `dataset merge` gives **0.46 → 0.61 (+0.15, 3/3)** — presets per defect type). Real public copies (MVTec metal_nut/screw/grid/bottle/hazelnut/carpet, Magnetic Tile, VisA, DTD) were rehearsed and benchmarked in v0.8 — see `TESTING.md` and the evidence section of `KNOWN-ISSUES.md`.
